...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname === "/sources/bulk") {
    if (req.method === "PATCH") {
      let body;
      try {
        body = await readJsonBody(req);
      } catch (error) {
        return sendJson(res, 400, { error: error.message, message: "Request body must contain valid JSON." });
      }

      if (!Array.isArray(body.ids) || body.ids.length === 0) {
        return sendJson(res, 400, { error: "VALIDATION_ERROR", message: "ids must be a non-empty array." });
      }

      const ids = [...new Set(body.ids.map(String))];
      if (ids.length > 500) {
        return sendJson(res, 400, { error: "VALIDATION_ERROR", message: "Maximum 500 records allowed per bulk operation." });
      }
      ...
      if (body.action === "enable" || body.action === "disable") {
        ...
        values.push(body.action === "enable");
      } else if (body.action === "set_priority") {
        if (!validatePriority(body.priority)) {
          return sendJson(res, 400, { error: "VALIDATION_ERROR", message: "priority must be an integer from 1 to 5." });
        }
        updates.push(`priority = $1`);
        values.push(body.priority);
      } else {
        return sendJson(res, 400, { error: "VALIDATION_ERROR", message: "action must be enable, disable, or set_priority." });
      }
      ...
      try {
        const updateResult = await pool.query(
          `UPDATE sources SET ${updates.join(", ")} WHERE id IN (${placeholders}) RETURNING id`,
          values
        );
        return sendJson(res, 200, { data: { updated_count: updateResult.rowCount } });
      } catch (error) {
        return handleDatabaseError(res, error);
      }
    }
    ...
    if (req.method === "DELETE") {
      ...
      try {
        const result = await pool.query(`DELETE FROM sources WHERE id IN (${placeholders}) RETURNING id`, ids);
        return sendJson(res, 200, { data: { deleted_count: result.rowCount } });
      } catch (error) {
        return handleDatabaseError(res, error);
      }
    }
  ...
  // =======================================================
  if (req.method === "PATCH" && pathname === "/contents/bulk/status") {
    let body;
    try {
      body = await readJsonBody(req);
    } catch (error) {
    ...
    if (!Array.isArray(body.content_ids) || body.content_ids.length === 0) {
      return sendJson(res, 400, { error: "VALIDATION_ERROR", message: "content_ids must be a non-empty array." });
    }

    const contentIds = [...new Set(body.content_ids.map(String))];
    if (contentIds.length > 500) {
      return sendJson(res, 400, { error: "VALIDATION_ERROR", message: "Maximum 500 records allowed per bulk operation." });
    }

    const nextStatus = String(body.status || "").toUpperCase();
    if (!VALID_STATUSES.has(nextStatus)) {
      return sendJson(res, 400, { error: "INVALID_STATUS", message: `${nextStatus || "(empty)"} is not a valid workflow status.` });
    }
    ...
    try {
      ...
      const currentResult = await client.query(
        `SELECT id, content_id, status FROM contents WHERE content_id IN (${placeholders}) ORDER BY id FOR UPDATE`,
        contentIds
      );

      if (currentResult.rowCount !== contentIds.length) {
        await client.query("ROLLBACK");
        return sendJson(res, 404, { error: "CONTENT_NOT_FOUND", message: "One or more contents were not found." });
      }
      ...
      for (const row of currentResult.rows) {
        ...
        if (!allowed.includes(nextStatus)) {
          ...
          return sendJson(res, 409, {
            error: "INVALID_STATUS_TRANSITION",
            message: `Content ${row.content_id} (${row.status}) cannot transition to ${nextStatus}.`,
      ...
      await client.query(
        `UPDATE contents SET status = $1 WHERE content_id IN (${placeholders})`,
        [nextStatus, ...contentIds]
      );

      for (const row of currentResult.rows) {
        await client.query(
          `INSERT INTO content_status_history (content_id, from_status, to_status, changed_by, reason, metadata) VALUES ($1, $2, $3, $4, $5, $6::jsonb)`,
          [
            row.id,
            row.status,
            nextStatus,
  ...
  if (pathname === "/contents/bulk") {
    if (req.method === "PATCH") {
      ...
      if (updates.length === 0) {
        return sendJson(res, 400, { error: "NO_UPDATES", message: "Provide at least one editable field: priority." });
      }
      ...
      try {
        const updateResult = await pool.query(
          `UPDATE contents SET ${updates.join(", ")} WHERE content_id IN (${placeholders}) RETURNING content_id`,
          values
        );
        return sendJson(res, 200, { data: { updated_count: updateResult.rowCount } });
      } catch (error) {
    ...
    if (req.method === "DELETE") {
      ...
      const placeholders = contentIds.map((_, i) => `$${i + 1}`).join(", ");
      try {
        const result = await pool.query(`DELETE FROM contents WHERE content_id IN (${placeholders}) RETURNING content_id`, contentIds);
        return sendJson(res, 200, { data: { deleted_count: result.rowCount } });
      } catch (error) {
        return handleDatabaseError(res, error);
      }
