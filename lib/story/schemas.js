...
function validateOutlineShape(outline) {
  ...
  if (!isPlainObject(outline.canon_state_impact)) {
    ...
    const impact = outline.canon_state_impact;

    if (impact.state !== "PROPOSED_STATE_CHANGE") {
      issues.push(
        issue(
          "CANON",
          "CANON_STATE_NOT_PROPOSED",
          "canon_state_impact.state must be PROPOSED_STATE_CHANGE.",
          "canon_state_impact.state"
        )
      );
    }
    ...
    for (const field of [
      ...
      if (!isNonEmptyString(impact[field])) {
        issues.push(
          issue(
            "STRUCTURE",
            "OUTLINE_FIELD_MISSING",
            `canon_state_impact.${field} is required.`,
            `canon_state_impact.${field}`
          )
        );
      }
    }

    if (!isArray(impact.evidence_refs)) {
      issues.push(
        issue(
          "STRUCTURE",
          "OUTLINE_FIELD_MISSING",
          "canon_state_impact.evidence_refs must be an array.",
          "canon_state_impact.evidence_refs"
        )
      );
    }
  }
