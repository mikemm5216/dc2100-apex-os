...
// Task 3.6: rows persisted before the Outline/Script Integrated
// Signals fix carry no signal_contributions/coverage_status at all
// (NULL) -- they are tagged LEGACY_NO_COVERAGE for the UI, mirroring
// how Task 3.5E already tags pre-fix direction rows LEGACY_DIRECTION.
function coverageSchemaTag(row) {
  return row.coverage_status ? "INTEGRATED_COVERAGE" : "LEGACY_NO_COVERAGE";
}
...
function serializeOutline(row) {
  return {
    ...
    validation_status: row.validation_status,
    validation_issues: row.validation_issues,
    signal_contributions: row.signal_contributions,
    coverage_status: row.coverage_status,
    source_direction_ids: row.source_direction_ids,
    locked_beat_id: row.locked_beat_id,
    outline_schema: coverageSchemaTag(row),
...
function serializeScript(row) {
  return {
    ...
    signal_contributions: row.signal_contributions,
    coverage_status: row.coverage_status,
    source_outline_id: row.source_outline_id ? String(row.source_outline_id) : null,
    locked_beat_id: row.locked_beat_id,
    script_schema: coverageSchemaTag(row),
    locked_by: row.locked_by,
    locked_at: row.locked_at,
    superseded_at: row.superseded_at,
    created_at: row.created_at
...
async function getStoryRun(pool, runId) {
  ...
  // Review fix: a PASS validation_status alone is not enough -- a
  // legacy row with no signal_contributions/coverage_status/
  // locked_beat_id (see coverageSchemaTag) never went through the
  // Integrated Signals continuity checks and must never be lockable.
  const hasLockableOutline = activeOutlines.some(isIntegratedOutlineCoverage);

  const hasLockableScript = activeScripts.some(isIntegratedScriptCoverage);
