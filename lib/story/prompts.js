...
`.trim()
  ...
  const input = {
    ...
    locked_canon_identity_policy: candidateSnapshot.locked_canon
      ? {
          identities_locked: true,
          canon_driver_name: candidateSnapshot.locked_canon.canon_driver_name,
          canon_vehicle_name: candidateSnapshot.locked_canon.canon_vehicle_name,
          instruction: "Use both names exactly; do not rename, replace, or re-fictionalize them."
        }
      : null
  };
...
The selected direction(s) already established which evidence layers
(vehicle, country news, person/historical resonance, APEX) are
genuinely available for this candidate -- see
already_used_evidence_by_layer below. Any layer marked there must stay
visible in your own evidence_map with the same evidence id(s): do not
silently drop a signal the selected direction(s) already committed to
using. locked_beat_id below must not change -- this outline serves
that beat, never a different one.
...
Allowed evidence refs only:
${allowedEvidenceRefs.map(ref => `- ${ref}`).join("\n") || "- (none)"}
Do not output any other evidence ID in evidence_map.
`.trim()
  );
...
// complexity limit already worked around for Directions above. Three
// Script variants are therefore generated as three separate
// single-script calls (engine.js executeScriptsGeneration loops this
// prompt once per variant), never one call requesting all three.
...
produce EXACTLY ONE Script. variant_type must equal "${targetVariantType}"
exactly -- never any other value.

This call is one member of a three-variant batch (VEHICLE_FIRST,
WORLD_FIRST, CHARACTER_FIRST); the other variants are produced by
separate calls (see already_used_variant_types below for which ones
already exist in this batch). VEHICLE_FIRST / WORLD_FIRST /
CHARACTER_FIRST changes only which angle leads the hook and how the
script is packaged -- it must never change the underlying story
result: the same locked outline, the same outcome, the same evidence
layers used, and the same locked beat must hold across every variant
in the batch.

This script targets ${scriptLanguage} language output, 25-45 seconds
total, 5-8 shots, with the hook resolving in the first 3 seconds.
English scripts must have a vo_text of 70-110 words. It must have a
clear beginning, conflict, choice, outcome, and next-step hook -- not
just trailer lines or worldbuilding nouns. It must never contradict
the locked outline's outcome. At most one major proposed_state_changes
entry, always "state": "PROPOSED_STATE_CHANGE". Never imply an
official brand partnership, never portray a real person directly,
never copy a copyrighted character. Never write
"CANON_STATE_COMMITTED".

The locked outline already established which evidence layers (vehicle,
country news, person/historical resonance, APEX) are genuinely
available for this candidate. This script must keep each already-used
layer visible in its own evidence_map/shots evidence_refs with the same
evidence id(s) -- leading with "${targetVariantType}"'s own hook angle
must never silently drop another layer the outline already committed
to using. locked_beat_id below must not change from the locked
outline.

Allowed evidence refs only:
${allowedEvidenceRefs.map(ref => `- ${ref}`).join("\n") || "- (none)"}
Do not output any other evidence ID in evidence_map or shots[].evidence_refs.
`.trim()
  );
  ...
  const input = {
    ...
    locked_beat_id: lockedBeatId,
    allowed_evidence_refs: allowedEvidenceRefs,
    target_variant_type: targetVariantType,
    already_used_variant_types: usedVariantTypes,
    retry_feedback: retryFeedback || {
      previous_attempt_failed: false,
      validation_issues: []
    }
