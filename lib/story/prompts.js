...
available for this candidate. Every variant must keep each already-used
layer visible in its own evidence_map/shots evidence_refs with the same
evidence id(s) -- a variant emphasizing one layer (e.g. VEHICLE_FIRST)
must never silently drop another layer the outline already committed
to using. locked_beat_id below must not change across variants.
