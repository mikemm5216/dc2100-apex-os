...
// just sentences) so each chunk is a single claim, then check EVERY
// occurrence of a phrase/pattern within each clause independently --
// one negated occurrence must never excuse a later affirmative
// occurrence in the same clause.
...
// -- negation is scoped to the gap since the END of the previous
// occurrence of this same phrase (or the clause start, for the
// first), never the whole clause, so one negated occurrence can never
// launder a later affirmative one in the same clause.
function hasUnnegatedPhraseInClause(clause, phrase) {
  let windowStart = 0;

  for (const index of findAllOccurrences(clause, phrase)) {
    const localPrefix = clause.slice(windowStart, index);

    if (!NEGATION_CUE_PATTERN.test(localPrefix)) {
      return true;
    }

    windowStart = index + phrase.length;
  }

  return false;
}

// Same all-occurrences discipline for a regex pattern: every match
// within the clause is checked on its own, using only that match's
// own matched span (which already includes the subject-to-verb gap
// where a negation cue like "does not" would sit) to decide negation.
function hasUnnegatedPatternInClause(clause, pattern) {
  const globalPattern = new RegExp(pattern.source, pattern.flags.includes("g") ? pattern.flags : `${pattern.flags}g`);

  for (const match of clause.matchAll(globalPattern)) {
    if (!NEGATION_CUE_PATTERN.test(match[0])) {
      return true;
    }
  }
...
function runCanonValidator(payload) {
  ...
  // Every clause is checked for every occurrence independently -- a
  // negated traffic clause earlier in the text (or earlier in the same
  // clause) must never excuse a real affirmative occurrence later on.
  const trafficDecidesResultViolated = splitIntoClauses(text).some(clause =>
    hasUnnegatedPatternInClause(clause, TRAFFIC_DECIDES_RESULT_PATTERN)
  );
...
function runIPValidator(payload, context = {}) {
  ...
  // independently -- "No official partnership existed before, but now
  // the team has an official partnership with Toyota." must still
  // BLOCK on its second, affirmative clause even though the first
  // clause's "no" negates its own occurrence.
  const clauses = splitIntoClauses(text);

  for (const phrase of OFFICIAL_PARTNERSHIP_PHRASES) {
    const violated = clauses.some(clause => hasUnnegatedPhraseInClause(clause, phrase));
