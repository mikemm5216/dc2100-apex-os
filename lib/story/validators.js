...
// the same trigger words as a real violation, just negated. Segment
// the narrative text into sentence-ish chunks so a negation cue in
// one sentence can never excuse a real violation in a different one,
// then require the negation cue to appear BEFORE the flagged phrase
// within the same segment (matching how every provided negated
// example is actually phrased).
...
function isNegatedBeforeIndex(segment, index) {
  return NEGATION_CUE_PATTERN.test(segment.slice(0, index));
}
...
function runCanonValidator(payload) {
  ...
  const trafficDecidesResultViolated = splitIntoSegments(text).some(segment => {
    const match = TRAFFIC_DECIDES_RESULT_PATTERN.exec(segment);
    // The affirmative pattern's [^.]{0,40} gap between subject and verb
    // is wide enough to also match "traffic does not decide..." --
    // a negation cue anywhere inside the matched span (e.g. between
    // "traffic" and "decide") means this segment is the safe, negated
    // form, not a real violation.
    return Boolean(match) && !NEGATION_CUE_PATTERN.test(match[0]);
  });
...
function runIPValidator(payload, context = {}) {
  ...
  for (const phrase of OFFICIAL_PARTNERSHIP_PHRASES) {
    const violated = segments.some(segment => {
      ...
      // "No official partnership is implied" / "not officially
      // sponsored by" / "without an official partnership" all put the
      // negation cue immediately before the phrase, in the same
      // sentence -- require that same shape before treating a match
      // as safe, so a negation cue in an unrelated sentence can never
      // excuse a real violation.
      return index !== -1 && !isNegatedBeforeIndex(segment, index);
    });
