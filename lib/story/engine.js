...
async function executeScriptsGeneration(pool, run, deps) {
  ...
  try {
    ...
    for (
      ...
      const generation = await deps.generateJson({
        ...
        temperature: 0.8,
        validate: parsed => {
          const parsedScripts = Array.isArray(parsed) ? parsed : parsed?.scripts;
          const issues = validateScriptBatchShape(parsedScripts, {
            language: run.script_language
          });
          return { valid: issues.length === 0, errors: issues.map(i => i.message) };
        }
      ...
      const candidateScripts = Array.isArray(generation.data)
        ? generation.data
        : generation.data.scripts;

      const { batch, perScript } = validateScriptBatchWithContext(
        candidateScripts,
        validationContext
      );

      const allPass =
        batch.validation_status === "PASS" &&
      ...
      const allIssues = [
        ...batch.issues,
        ...perScript.flatMap(result => result.issues)
      ];

      retryFeedback = {
        previous_attempt_failed: true,
        validation_issues: allIssues.map(item => ({
          code: item.code,
          message: item.message,
          path: item.path,
          allowed_evidence_refs:
  ...
  const finalAttempt = attempts[attempts.length - 1];
  const { batch: finalBatch, perScript } = finalAttempt;

  // Review fix: batch and perScript are validated together (retry
  // requires BOTH to be PASS -- see allPass above), but a batch-level
  // failure (duplicate/missing variant_type, wrong variant count) says
  // nothing about whether an individual script's OWN shape/content
  // happened to validate cleanly. Persisting per-script status alone
  // after retries are exhausted would let a structurally-fine script
  // from an incomplete batch end up PASS and lockable on its own --
  // the batch is the unit of validity, so a batch failure blocks
  // every row from that final attempt, not just the offending one(s).
    ...
    async client => {
      ...
      for (const attempt of attempts) {
        await recordGenerationAttempt(client, {
          ...
          attemptNumber: attempt.attemptNumber,
          status:
            attempt.batch.validation_status === "PASS" &&
            attempt.perScript.every(result => result.validation_status === "PASS")
              ? "SUCCESS"
              : "FAILED",
          issueCodes: [
            ...attempt.batch.issues.map(item => item.code),
            ...attempt.perScript.flatMap(result =>
