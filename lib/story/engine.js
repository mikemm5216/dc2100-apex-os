...
const { generateJson } = require("./provider");
const {
...
async function buildCandidateSnapshot(pool, fusionCandidateId) {
  ...
  return {
    ...
    country: row.country_id
      ? { id: String(row.country_id), code: row.country_code, name: row.country_name }
      : null,
    ...
    person: row.person_id
      ...
      : null,
    vehicle_person_link: row.vehicle_person_link_id
      ? {
    ...
    no_person_signal: missingSignals.includes(
...
async function fetchRunForUpdate(client, runId) {
  const result = await client.query(
    `SELECT * FROM story_pipeline_runs WHERE id = $1 FOR UPDATE`,
    [runId]
  );
...
async function approveCandidate(pool, runId, body) {
...
async function selectDirection(pool, runId, body) {
  ...
  try {
    ...
    await insertEvent(client, runId, "DIRECTION_SELECTED", "DIRECTIONS", {
      ...
      merge_notes: body.merge_notes || null
    });
...
async function claimNextStoryRun(
  ...
  try {
    ...
    if (nextAttemptCount > STORY_MAX_STAGE_ATTEMPTS) {
...
async function recordGenerationAttempt(pool, {
  runId,
  stage,
  artifactVersion,
  provider,
...
async function failRunForStage(pool, runId, stage, error) {
  await pool.query(
    `
      UPDATE story_pipeline_runs
      SET
    ...
    [
      stage,
      error?.code || error?.storyCode || "GENERATION_FAILED",
      String(error?.message || "Unknown generation failure").slice(0, 2000),
      runId
    ]
  );

  await insertEvent(pool, runId, "STORY_GENERATION_FAILED", stage, {
    error_code: error?.code || error?.storyCode || "GENERATION_FAILED",
...
async function executeDirectionsGeneration(pool, run, deps) {
  ...
  const { systemPrompt, input } = buildDirectionsPrompt({
    canonBundle,
  ...
  try {
    ...
    await recordGenerationAttempt(pool, {
      runId: run.id,
      stage: "DIRECTIONS",
    ...
    await failRunForStage(pool, run.id, "DIRECTIONS", error);

    return { outcome: "RUN_FAILED", stage: "DIRECTIONS" };
  }
  ...
  const updateResult = await pool.query(
    `
      ...
      SET
        ...
        lease_expires_at = NULL,
        updated_at = NOW()
      WHERE id = $1 AND status = 'GENERATING_DIRECTIONS' AND worker_id = $2
      RETURNING *
    `,
  ...
  await insertEvent(pool, run.id, "STORY_DIRECTIONS_GENERATED", "DIRECTIONS", {
    version
  });

  return { outcome: "STAGE_ADVANCED", stage: "DIRECTIONS", run: updateResult.rows[0] };
}
...
async function executeOutlineGeneration(pool, run, deps) {
  ...
  const { systemPrompt, input } = buildOutlinePrompt({
    canonBundle,
    candidateSnapshot: snapshot,
    selectedDirections,
    selectionMode: selectedIds.length === 2 ? "MERGE" : "SINGLE",
    reviewLanguage: run.review_language,
    forbiddenElements: run.forbidden_elements,
    revisionNotes
  });

  const version = await nextArtifactVersion(pool, run.id, "story_outlines");
  ...
  try {
    ...
    await failRunForStage(pool, run.id, "OUTLINE", error);

    return { outcome: "RUN_FAILED", stage: "OUTLINE" };
  }
  ...
  const updateResult = await pool.query(
    `
      ...
      SET
        ...
        lease_expires_at = NULL,
        updated_at = NOW()
      WHERE id = $1 AND status = 'GENERATING_OUTLINE' AND worker_id = $2
      RETURNING *
    `,
    [run.id, run.worker_id]
  );
  ...
  await insertEvent(pool, run.id, "STORY_OUTLINE_GENERATED", "OUTLINE", {
    version
  });

  return { outcome: "STAGE_ADVANCED", stage: "OUTLINE", run: updateResult.rows[0] };
}
...
async function executeScriptsGeneration(pool, run, deps) {
  ...
  const { systemPrompt, input } = buildScriptsPrompt({
    canonBundle,
    candidateSnapshot: snapshot,
    outline,
  ...
  const version = await nextArtifactVersion(pool, run.id, "story_scripts");
  ...
  try {
    ...
    await failRunForStage(pool, run.id, "SCRIPTS", error);

    return { outcome: "RUN_FAILED", stage: "SCRIPTS" };
  }
  ...
  const updateResult = await pool.query(
    `
      ...
      SET
        ...
        lease_expires_at = NULL,
        updated_at = NOW()
      WHERE id = $1 AND status = 'GENERATING_SCRIPTS' AND worker_id = $2
      RETURNING *
    `,
    [run.id, run.worker_id]
  );
  ...
  await insertEvent(pool, run.id, "STORY_SCRIPTS_GENERATED", "SCRIPTS", {
    version
  });

  return { outcome: "STAGE_ADVANCED", stage: "SCRIPTS", run: updateResult.rows[0] };
}
