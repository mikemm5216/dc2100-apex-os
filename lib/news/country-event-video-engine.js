...
// Persists this run's matched video AND, in the same
// transaction, removes every other country_event_video_signals
// row for the same country -- a stale video from a prior event
// (a different country_news_signal_id) must never keep showing
// once a new event's video has matched. The upsert always runs
// BEFORE the delete, so a failed insert never leaves the
...
async function upsertCountryEventVideoSignal(
  ...
  try {
    ...
    const newRowId = result.rows[0].id;

    await client.query(
      `
        DELETE FROM country_event_video_signals
        WHERE country_id = $1
          AND id != $2
      `,
      [countryId, newRowId]
...
// Confirmed NO_MATCH only: the search completed normally (no
// exception) but no candidate qualified. Any video persisted
// for a PRIOR event on this country must not keep appearing as
// if it were still current, so it is removed here. This must
// never run on an actual error (quota, timeout, DB failure) --
// callers only reach this after a successful search + evaluate
// pass with zero qualifying candidates.
async function clearCountryEventVideoSignal(pool, countryId) {
  await pool.query(
    `
      DELETE FROM country_event_video_signals
      WHERE country_id = $1
...
async function processCountryEntity(
  ...
  if (
    ...
    await clearCountryEventVideoSignal(pool, country.id);
  ...
  if (candidates.length === 0) {
    // Confirmed NO_MATCH: the search + evaluate pass completed
    // normally with zero qualifying candidates, so any video
    // left over from a prior event on this country must be
    // cleared -- never surfaced as if it were still current.
    await clearCountryEventVideoSignal(pool, country.id);
    state.noMatchEntityCount += 1;
    state.entityResults.push(result);
    return;
