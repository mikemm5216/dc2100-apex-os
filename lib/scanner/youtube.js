...
// Historical mode only: walks the ENTIRE uploads playlist,
// one page at a time, sequentially (never parallel, so a
// single source can never burst the YouTube quota). Stops
// when nextPageToken runs out, or when maxPages is hit --
// in which case `truncated: true` tells the caller this
// source's history is not fully scanned, and history_complete
// must never be claimed for it.
async function fetchAllUploadVideoIds(
  ...
  do {
    ...
    if (pageToken && pagesScanned >= maxPages) {
      truncated = true;
      pageToken = null;
    }
  } while (pageToken);
...
// search.list: type=video, order=viewCount, optionally
// scoped to publishedAfter. Callers must issue these
// sequentially (never Promise.all across entities) so a
// single run can never burst the YouTube quota -- this
// function itself only ever performs ONE request.
async function searchVideos(
  ...
  const boundedMaxResults = Math.min(
    20,
    Math.max(10, Number(maxResults) || 20)
  );
