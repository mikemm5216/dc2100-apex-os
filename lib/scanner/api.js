...
//
// This ranks INDIVIDUAL VIDEOS, not vehicles-by-aggregate.
// A signal is one specific video, and what this radar looks
// for is a single, already-market-proven video Hook -- never
// the sum of every video a vehicle has ever appeared in.
//
// Each distinct resolved vehicle keeps exactly one row: its
// own single highest-viewed eligible video. Top 10 is that
// video's actual views, DESC. No fusion_score, no rank_score,
// no growth_velocity, and no SUM(views) anywhere in this
// query. Structurally mirrors Person Radar's relationship_scope
// pattern (ONE_YEAR ⊂ TEN_YEARS ⊂ ALL_TIME).
// =========================================================
...
async function listVehicleHistoricalRanking(
  ...
  // filtered_signals: every eligible VIDEO (not vehicle).
  // top_video_per_vehicle keeps exactly one row per vehicle --
  // its own single highest-viewed video, tie-broken by
  // published_at DESC then signal id ASC. This is the
  // DISTINCT ON equivalent of
  // ROW_NUMBER() OVER (PARTITION BY resolved_vehicle_id
  //   ORDER BY views DESC, published_at DESC, id ASC) = 1.
  // vehicle_signal_counts is reference-only (how many eligible
  // videos exist for the vehicle) -- it never feeds the ORDER BY.
  const cte = `
    filtered_signals AS (
      SELECT
        sig.id,
    ...
    vehicle_signal_counts AS (
      SELECT
        resolved_vehicle_id AS vehicle_id,
        COUNT(DISTINCT id) AS vehicle_signal_count
      FROM filtered_signals
  ...
  const listResult = await pool.query(
    `
      ...
      SELECT
        ...
        vsc.vehicle_signal_count

      FROM top_video_per_vehicle tv

      JOIN vehicles veh
        ON veh.id = tv.vehicle_id

      LEFT JOIN sources src
        ON src.id = tv.source_id

      LEFT JOIN vehicle_signal_counts vsc
        ON vsc.vehicle_id = tv.vehicle_id

      ORDER BY
        tv.video_views DESC,
        tv.published_at DESC,
        tv.signal_id ASC
  ...
  // even though this endpoint paginates the one-row-per-vehicle
  // result, not the raw signal rows.
  const totalCountResult = await pool.query(
    `
      WITH ${cte}
      SELECT
        COUNT(DISTINCT vehicle_id)::int AS total_count
      FROM top_video_per_vehicle
    `
  );
...
//
// Evidence detail for ONE vehicle's single highest-viewed
// eligible video -- the exact video that earned it a Top 10
// slot, never a list of every video the vehicle has ever
// appeared in.
// =========================================================
...
async function getVehicleHistoricalDetail(
  ...
  const topVideoResult = await pool.query(
    `
      SELECT
        sig.id AS signal_id,
  ...
  const signalCountResult = await pool.query(
    `
      SELECT
        COUNT(DISTINCT sig.id)::int
  ...
  return response(200, {
    data: {
      ...
      vehicle_signal_count: Number(
        signalCountResult.rows[0]
          ?.vehicle_signal_count || 0
      ),
      scan_summary: scanSummary,
      top_video: topVideoResult.rows[0] || null
    }
  });
}
