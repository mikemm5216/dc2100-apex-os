...
// vehicle with no eligible person produces NO_PERSON_SIGNAL
...
async function processVehicle(pool, run, vehicle, state) {
  const options = state.options;

  const countryNews = await selectEligibleCountryNews(
    pool,
    {
      countryId: vehicle.country_id,
      newsWindowHours: options.newsWindowHours,
      maxNewsPerVehicle: options.maxNewsPerVehicle
    }
  );
  ...
  if (countryNews.length === 0) {
    ...
    state.vehicleResults.push({
      vehicle_code: vehicle.vehicle_code,
      status: "SKIPPED",
      code: "NO_COUNTRY_NEWS_SIGNAL",
      message:
        "No country-matched news signal within window."
    });
    ...
    state.errors.push({
      ...
      code: "NO_COUNTRY_NEWS_SIGNAL",
      message:
        `No country-matched news signal for ${vehicle.vehicle_code} (country ${vehicle.country_code || "unknown"}).`
    });

    return;
  }

  const personLinks = await selectEligiblePersonLinks(
    pool,
    {
      vehicle,
      maxPeoplePerVehicle: options.maxPeoplePerVehicle
    }
  );

  const personCombos =
    personLinks.length > 0 ? personLinks : [null];

  let candidateCountForVehicle = 0;

  for (const newsSignal of countryNews) {
    for (const personLink of personCombos) {
      const candidate = buildCandidate({
        vehicle,
        countryNewsSignal: newsSignal,
        personLink
      });

      const saved = await upsertFusionCandidate(
        pool,
        run.id,
        candidate
      );

      candidateCountForVehicle += 1;
      ...
      if (candidate.personLinkTier === "EXACT_VEHICLE") {
        state.exactVehicleCount += 1;
      } else if (
        candidate.personLinkTier === "SAME_SERIES"
      ) {
        state.sameSeriesCount += 1;
      } else if (
        candidate.personLinkTier === "SAME_BRAND"
      ) {
        state.sameBrandCount += 1;
      } else {
        state.noPersonSignalCount += 1;
      }
    }
  }
  ...
  state.vehicleResults.push({
    vehicle_code: vehicle.vehicle_code,
    status: "COMPLETED",
    country_news_count: countryNews.length,
    person_link_count: personLinks.length,
    candidate_count: candidateCountForVehicle
  });
}
...
async function executeFusionRun(
  ...
  const vehicles = await selectVehicleTrafficAnchors(
    pool,
    options
  );
  ...
  if (vehicles.length === 0) {
    ...
    error.code = NO_QUALIFIED_VEHICLE_TRAFFIC_ERROR;
    ...
    return {
      runId: String(run.id),
      status: "FAILED",
      errorCode: NO_QUALIFIED_VEHICLE_TRAFFIC_ERROR,
      ...state
    };
  }

  state.vehicleCount = vehicles.length;

  await updateRunProgress(pool, run.id, state);

  for (const vehicle of vehicles) {
    try {
      await processVehicle(pool, run, vehicle, state);

      if (onVehicleCompleted) {
      ...
      state.vehicleResults.push({
        ...
        status: "FAILED",
        message: String(
          error?.message || "Unknown vehicle failure"
        ).slice(0, 500)
      });
