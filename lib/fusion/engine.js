  ...
  const result = await pool.query(`
    WITH eligible AS (
      SELECT ps.*, v.code AS vehicle_code, v.name AS vehicle_name,
        COALESCE(NULLIF(TRIM(v.manufacturer), ''), ps.evidence->>'vehicle_brand')
          AS vehicle_brand,
        LOWER(REGEXP_REPLACE(TRIM(COALESCE(
          NULLIF(TRIM(v.manufacturer), ''),
          ps.evidence->>'vehicle_brand'
        )), '[[:space:]]+', ' ', 'g'))
        ...
        ROW_NUMBER() OVER (
          PARTITION BY LOWER(REGEXP_REPLACE(TRIM(COALESCE(
            NULLIF(TRIM(v.manufacturer), ''),
            ps.evidence->>'vehicle_brand'
          )), '[[:space:]]+', ' ', 'g'))
          ORDER BY
    ...
    WHERE brand_rn=1 AND vehicle_rn=1 AND person_rn=1
    ORDER BY
      CASE pair_status WHEN 'PROVEN_PAIR' THEN 0 ELSE 1 END,
      joint_video_views DESC NULLS LAST,
      CASE pair_specificity
        WHEN 'EXACT_MODEL' THEN 0 WHEN 'SAME_SERIES' THEN 1 ELSE 2
      END,
      vehicle_anchor_views DESC,
      joint_video_published_at DESC NULLS LAST,
      normalized_vehicle_brand ASC,
      vehicle_id ASC, person_id ASC
    LIMIT $2
  `, [id, maxVehicles]);
  return result.rows;
}
...
async function finalizeRun(pool, runId, state) {
  ...
  let supersedeEligible = false;
  if (completed) {
    const eligibility = await pool.query(`
      SELECT
        fr.pair_run_id IS NOT NULL
          AND pr.status='COMPLETED'
          AND COALESCE((pr.summary->>'target_reached')::boolean,FALSE)
          AND COUNT(fc.id) > 0
          AND COUNT(fc.id)=COUNT(fc.person_id)
          AND COUNT(fc.id)=COUNT(DISTINCT fc.vehicle_id)
          AND COUNT(fc.id)=COUNT(DISTINCT fc.person_id)
          AND COUNT(fc.id)=COUNT(DISTINCT LOWER(COALESCE(
            NULLIF(vps.evidence->>'normalized_vehicle_brand',''),
            NULLIF(vps.evidence->>'vehicle_brand','')
          )))
          AND BOOL_AND(COALESCE(vps.selected,FALSE))
          AND BOOL_AND(fc.pair_run_id=fr.pair_run_id)
          AND BOOL_AND(fc.vehicle_person_pair_signal_id=vps.id)
          AS eligible
      FROM fusion_runs fr
      JOIN vehicle_person_pair_runs pr ON pr.id=fr.pair_run_id
      LEFT JOIN vehicle_fusion_candidates fc ON fc.run_id=fr.id
      LEFT JOIN vehicle_person_pair_signals vps
        ON vps.id=fc.vehicle_person_pair_signal_id
      WHERE fr.id=$1
      GROUP BY fr.pair_run_id,pr.status,pr.summary
    `, [runId]);
    supersedeEligible = eligibility.rows[0]?.eligible === true;
  }
