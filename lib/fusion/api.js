...
function validateFusionRunPayload(body) {
  ...
  if (body.allow_partial_pair_run !== undefined &&
    typeof body.allow_partial_pair_run !== "boolean") {
    return validationError("allow_partial_pair_run must be a boolean.");
  }
  const contentMode = String(body.content_mode || "STATION_GUEST").toUpperCase();
  if (!["LOCKED_CANON", "STATION_GUEST"].includes(contentMode)) {
    return validationError("content_mode must be LOCKED_CANON or STATION_GUEST.");
  }
  ...
  return {
    value: {
      ...
      allow_partial_pair_run: body.allow_partial_pair_run === true,
      content_mode: contentMode,
      station_search_budget: contentMode === "STATION_GUEST" ? 30 : 0
    }
  };
}
...
async function createFusionRun(pool, body) {
  ...
  if (requestPayload.content_mode === "LOCKED_CANON") {
    return response(409, {
      error: "LOCKED_CANON_BYPASSES_FUSION",
      message: "LOCKED_CANON uses a locked roster slot and country news matrix; it never queues Pair or Fusion."
    });
  }
  ...
  if (requestPayload.pair_run_id) {
    const pairResult = await pool.query(`
      SELECT id,status,summary
      FROM vehicle_person_pair_runs
      WHERE id=$1
    `, [requestPayload.pair_run_id]);
    if (!pairResult.rowCount) {
      return response(404, { error: "PAIR_RUN_NOT_FOUND" });
    }
    const pairRun = pairResult.rows[0];
    const ready = pairRun.status === "COMPLETED" &&
      pairRun.summary?.target_reached === true;
    if (!ready && !requestPayload.allow_partial_pair_run) {
      return response(409, {
        error: "PAIR_RUN_NOT_READY_FOR_FUSION",
        pair_run: pairRun
      });
    }
  }
  ...
  const insertResult = await pool.query(
    `
      ...
      RETURNING
        ...
        created_at
    `,
    [JSON.stringify(requestPayload), requestPayload.content_mode]
  );
  ...
  const result = await pool.query(
    `
      ...
      WHERE fr.status = 'COMPLETED'
        AND fr.superseded_by_run_id IS NULL
