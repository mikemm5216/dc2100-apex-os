...
function selectTopDistinctBrandPairs(pairs, targetPairs) {
  return selectBestPairPerBrand(pairs)
    .sort((a, b) => compareUsablePairs(a, b, { global: true }))
    .slice(0, targetPairs);
}
...
async function persistUsablePair(pool, runId, pair) {
  if (pair.pairStatus === "PROVEN_PAIR") {
    return persistProvenPair(pool, runId, pair.anchor, pair.link, pair);
  }
...
async function scanBrandAnchorBatches({
  ...
  for (let offset = 0; offset < anchors.length; offset += batchSize) {
    ...
    for (const anchor of batch) {
      brandsScanned += 1;
      const pair = await scanAnchor(anchor);
      if (pair) usablePairs.push(pair);
    }
    batchesCompleted += 1;
    if (selectBestPairPerBrand(usablePairs).length >= targetPairs) break;
  }
...
async function executeVehiclePersonPairRun(pool, run, { apiKey } = {}) {
  ...
  const brandPairs = scanOutcome.usablePairs;
  const brandBatchesCompleted = scanOutcome.batchesCompleted;
  state.entitiesAttempted = scanOutcome.brandsScanned;

  const selectedPairs = selectTopDistinctBrandPairs(brandPairs, options.targetPairs);
  const results = [];
  for (const pair of selectedPairs) {
    try {
      const saved = await persistUsablePair(pool, run.id, pair);
      results.push({
  ...
  const targetReached = results.length === options.targetPairs &&
    distinctSelectedBrandCount === results.length;
  const summary = {
    ...
    usable_brand_pairs: selectBestPairPerBrand(brandPairs).length,
    ...
    result_code: targetReached ? "TARGET_REACHED" : "INSUFFICIENT_DISTINCT_BRAND_PAIRS",
