    ...
    const result = await pool.query(
      ...
      [
        ...
        JSON.stringify({
          ...
          f1_driver: person.f1Driver === true,
          official_sources: [...new Set([
            person.lifeStatusSource,
            ...person.associations.map(item => item.source)
          ].filter(Boolean))]
        }),
        CATALOG_VERSION,
...
async function persistVerifiedF1CatalogLinks(
  pool,
  idsBySlug,
  { catalog = PERSON_CATALOG } = {}
) {
  for (const person of catalog) {
    if (person.f1Driver !== true) continue;
    const personRecord = idsBySlug.get(person.slug);
    if (!personRecord || personRecord.active === false) continue;

    for (const association of person.associations) {
      if (!association.source) continue;
      await pool.query(
        `
          INSERT INTO vehicle_person_links (
            person_id, vehicle_id, vehicle_brand, vehicle_series,
            vehicle_model, relation_type, link_confidence,
            link_method, link_evidence
          )
          VALUES ($1, NULL, $2, $3, $4, $5, $6, 'CATALOG', $7::jsonb)
          ON CONFLICT (
            person_id,
            COALESCE(vehicle_brand, ''),
            COALESCE(vehicle_series, ''),
          ...
          DO UPDATE SET
            link_confidence=GREATEST(
              COALESCE(vehicle_person_links.link_confidence, 0),
        ...
        [
          ...
          JSON.stringify({
            ...
            relationship: "CONSTRUCTOR_OR_WORKS_TEAM",
            f1_driver: true,
