...
// Bounds how many candidate signals are pulled into JS for
// the direct-mention alias scan. Candidates are pre-sorted by
// views DESC, so the first alias match found is already the
// correct global maximum as long as the true best match is
// within this bound -- a deliberate, documented trade-off
// against scanning the entire signals table per read.
const DIRECT_HOOK_CANDIDATE_LIMIT = 5000;
...
// Bounded candidate pool for direct-mention alias scanning,
// pre-sorted so the first alias match per person is already
// the correct single highest-viewed video.
async function fetchDirectHookCandidates(
  pool,
  { historyScope, format }
) {
  const conditions = personDualVideoConditions({
  ...
  const result = await pool.query(
    `
      ...
      ORDER BY
        ...
        published_at DESC,
        external_id ASC
      LIMIT ${DIRECT_HOOK_CANDIDATE_LIMIT}
    `
  );
...
function findDirectMentionMatch(person, candidates) {
  const terms = [
...
async function upsertPersonDirectVideoSignal(
  pool,
  { personId, signalId, matchedAlias, field, evidence }
) {
  await pool.query(
    `
      ...
      ON CONFLICT (person_id, signal_id)
      DO UPDATE SET
        matched_alias = EXCLUDED.matched_alias,
        direct_mention_field =
          EXCLUDED.direct_mention_field,
...
function buildDirectHookVideoPayload(match) {
  if (!match) {
    return null;
  }

  const { candidate, matchedAlias, field } = match;

  return {
    signal_id: String(candidate.id),
    external_video_id: candidate.external_id,
    video_title: candidate.title,
    video_url: candidate.url,
    thumbnail_url: candidate.thumbnail_url,
    video_views: candidate.views,
    published_at: candidate.published_at,
    channel_title: candidate.channel_title,

    matched_alias: matchedAlias,
    direct_mention_field: field,
    direct_mention: true,
...
async function buildPersonDualVideoPacks(
  ...
  const [
    ...
    directHookCandidates
  ] = await Promise.all([
    fetchPersonAssociationCandidates(pool, {
      historyScope,
      format
    }),
    fetchDirectHookCandidates(pool, {
      historyScope,
      format
    })
  ]);
  ...
  for (const person of people) {
    ...
    const directMatch = findDirectMentionMatch(
      person,
      directHookCandidates
    );

    if (directMatch) {
      await upsertPersonDirectVideoSignal(pool, {
        personId: person.id,
        signalId: directMatch.candidate.id,
        matchedAlias: directMatch.matchedAlias,
        field: directMatch.field,
        evidence: {
          matched_alias: directMatch.matchedAlias,
          field: directMatch.field
        }
      });
    }
    ...
    const directHookVideo =
      buildDirectHookVideoPayload(directMatch);
