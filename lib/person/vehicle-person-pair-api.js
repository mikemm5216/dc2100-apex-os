...
function validateRunPayload(body = {}) {
  ...
  if (
    body.resume_run_id !== undefined &&
    !/^[1-9][0-9]*$/.test(String(body.resume_run_id))
  ) {
    return response(400, {
      error: "VALIDATION_ERROR",
      message: "resume_run_id must be a positive integer identifier."
    });
  }
  if (body.station_run_key !== undefined &&
    ...
    return response(400, {
      error: "VALIDATION_ERROR",
      message: "station_run_key contains invalid characters."
    });
  }
...
async function createVehiclePersonPairRun(pool, body) {
  ...
  if (options.resumeRunId) {
    const resumeResult = await pool.query(`
      SELECT id,status,request_payload,summary
      FROM vehicle_person_pair_runs
      WHERE id=$1
    `, [options.resumeRunId]);
    ...
    resumeRun = resumeResult.rows[0];
    if (!["FAILED", "PARTIAL"].includes(resumeRun.status)) {
      return response(409, {
        error: "PAIR_RUN_NOT_RESUMABLE",
        pair_run: resumeRun
      });
    }
  }
  const requestPayload = {
    ...
    max_search_queries: options.maxSearchQueries,
    resume_run_id: options.resumeRunId,
    station_run_key: options.stationRunKey
  };
  const result = await pool.query(`
    INSERT INTO vehicle_person_pair_runs(status, request_payload, resume_run_id)
    VALUES('QUEUED', $1::jsonb, $2)
    RETURNING id, status, request_payload, created_at
  `, [JSON.stringify(requestPayload), options.resumeRunId]);
...
async function listVehiclePersonPairSignals(pool, searchParams) {
  ...
  if (runId) {
    ...
    where.push("vpps.run_id=(SELECT id FROM vehicle_person_pair_runs WHERE status='COMPLETED' AND superseded_by_run_id IS NULL AND COALESCE((summary->>'target_reached')::boolean,FALSE) ORDER BY completed_at DESC,id DESC LIMIT 1)");
  }
  if (pairStatus !== "ALL") {
