...
function validateRunPayload(body = {}) {
  ...
  for (const [field, max] of [["max_vehicles", 50], ["max_people_per_vehicle", 10]]) {
    if (
      body[field] !== undefined &&
      (!Number.isInteger(body[field]) || body[field] < 1 || body[field] > max)
    ) {
      return response(400, {
        error: "VALIDATION_ERROR",
...
async function createVehiclePersonPairRun(pool, body) {
  ...
  const requestPayload = {
    ...
    max_vehicles: options.maxVehicles,
...
async function listVehiclePersonPairSignals(pool, searchParams) {
  ...
  const values = [];
  const where = ["vpps.selected=TRUE"];
  if (runId) {
    values.push(runId);
